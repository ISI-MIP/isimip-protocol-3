...
::: show sector=marine-fishery_regional

    #### Ocean regions

    ::: table identifier=ocean_region caption="Table 4.8: Ocean region specifiers (`ocean-region`)."
...
::: hide sector=agriculture,forestry,lakes_local,marine-fishery_regional,water_regional
...
::: show sector=peat
    ...
    All variables should be reported separately for the peat fraction of the grid cell if they are calculated separately for peat and non-peat grid-cell fractions, with a peat type `-<pt>` (`-peat`, `-drainpeat`, `-restorepeat`, or `-minl`) extension to the variable name. This extension is in addition to the extension expressing the Plant Functional Type (PFT) and needs to put before it.
