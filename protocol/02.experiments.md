...
::: show simulation_round=ISIMIP3b
    ...
    Please note that there is no "pre-industrial control run" from 1601-1849 for these experiments (`2015soc`, `nat`, `nofire`, `nondep`, `nondep2015co2`, `histco2`) and hence the spin-up links directly to the historical period.
...
::: show simulation_round=ISIMIP3b sector=water_global
    ...
    For the ISIMIP3b Group III simulations, modelling teams in the global water sector are requested to first use the land use and irrigation direct human forcings, as well as non-irrigation water use for the models that need it. These are the `default` Group III simulations for the sector. These include adaptation and no adaptation runs; though note that non-irrigation water use forcing is the same for both.
